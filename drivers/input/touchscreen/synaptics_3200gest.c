...
{
	if (strcmp(s2w, "1") == 0) {
		printk(KERN_INFO "[cmdline_s2w]: Sweep2Wake mode 1. | s2w='%s'", s2w);
		s2w_switch = 1;
	} else if (strcmp(s2w, "2") == 0) {
		printk(KERN_INFO "[cmdline_s2w]: Sweep2Wake mode 2. | s2w='%s'", s2w);
		s2w_switch = 2;
	} else if (strcmp(s2w, "3") == 0) {
		printk(KERN_INFO "[cmdline_s2w]: Sweep2Wake mode 3. | s2w='%s'", s2w);
		s2w_switch = 3;
	} else if (strcmp(s2w, "4") == 0) {
		printk(KERN_INFO "[cmdline_s2w]: Sweep2Wake mode 4. | s2w='%s'", s2w);
		s2w_switch = 4;
	} else if (strcmp(s2w, "5") == 0) {
		printk(KERN_INFO "[cmdline_s2w]: Sweep2Wake mode 5. | s2w='%s'", s2w);
		s2w_switch = 5;
			...
			if (ts->debug_log_level & 0x2)
				...
				if ((((ts->finger_count > 0)?1:0) == 0) && (s2w_switch > 0)) {
					...
					if (s2w_switch > 2) {
						...
						if (((scr_suspended == false) || (s2w_switch == 5)) && (minx > 460) && (minx < 660) && (maxx > 1000)) {
							if (exec_count) {
									   ...
									   ((finger_data[i][0] < prevx) &&
										...
										if ((finger_data[i][0] < prevx) &&
											...
											if (finger_data[i][0] < 270) {
												...
												} else if ((s2w_wakestat == 1) && (exec_count == true)) {
													//toggle panel OFF for deep sleep
													printk(KERN_INFO "[sweep2wake]: deep sleep");
													//disable_irq_wake(ts->client->irq);
													if (ts->power)
														ts->power(0);
													else {
														if (ts->psensor_status > 0) {
															ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x01, CONTROL_BASE), 0x02); /* sleep without calibration*/
															if (ret < 0)
																i2c_syn_error_handler(ts, ts->i2c_err_handler_en, "sleep: 0x02", __func__);
														} else {
															ret = i2c_syn_write_byte_data(ts->client, get_address_base(ts, 0x01, CONTROL_BASE), 0x01); /* sleep */
															if (ret < 0)
...
*/
	...
	//printk(KERN_INFO "num fingers = %x\n", ts->finger_support);
...
static int synaptics_ts_suspend(struct i2c_client *client, pm_message_t mesg)
{
	...
	if (s2w_switch > 0) {
		...
		if ((s2w_switch == 2) || (s2w_switch == 3) || (s2w_switch == 5)) {
	...
	if (ts->use_irq) {
		if ((s2w_switch == 0) || (s2w_switch == 1) || (s2w_switch == 4)) {
